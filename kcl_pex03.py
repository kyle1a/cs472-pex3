# Team Members: Jared Barrett and Kyle La
import logging
import time
import cv2
import random
...
DEFAULT_MAX_CONFIRM_ATTEMPTS = 3
MAX_ANGULAR_VELOCITY = .5

# Font for use with the information window
IMG_FONT = cv2.FONT_HERSHEY_SIMPLEX
IMG_SNAPSHOT_PATH = '/media/usafa/aux_data/pex03_mission/kcl_pex'
MISSION_VIRTUAL_MODE = False
...
class DroneMission:
    ...
    def switch_mission_to_confirm_mode(self):
        ...
        # allow drone to reposition and stabilize
        # time.sleep(15)
    ...
    def adjust_to_target_center(self, target_point, frame_write=None):
        ...
        pixel_forgiveness = 4  # TODO: you decide what "good enough" is to consider centered on x or y axis...
        ...
        if self.mission_mode == MISSION_MODE_TARGET:
            ...
            if target_point is not None \
                ...
                if not obj_track.total_track_misses:
                    ...
                    if (self.direction_y == "C"
                        ...
                        if abs(dx) > pixel_distance_threshold:
                            ...
                            xv = 1
                            x_duration = 3
                            # xv = min(xv,MAX_ANGULAR_VELOCITY)
                        ...
                        if self.direction_y != "C":  # If we are not centered on y-axis...
                            if self.direction_y == "F":
                                ...
                                if frame_write is not None:
                                    ...
                                    drone_lib.small_move_forward(self.drone, yv, y_duration)
                                    # pass  # TODO: remove "pass" when you've completed this condition.
                            else:
                                if frame_write is not None:
                                ...
                                #       or you can do a drone_lib.move_local as well.
                                drone_lib.small_move_back(self.drone, yv, y_duration)
                                # pass  # TODO: remove "pass" when you've completed this condition.
                        ...
                        if self.direction_x != "C":  # If we are not centered on x-axis...
                            ...
                            if self.direction_x == "R":
                                ...
                                #       or you can do a drone_lib.move_local as well.
                                drone_lib.small_move_right(self.drone, xv, x_duration)
                                # pass  # TODO: remove "pass" when you've completed this condition.
                            else:
                                if frame_write is not None:
                                ...
                                #       or you can do a drone_lib.move_local as well.
                                drone_lib.small_move_left(self.drone, xv, x_duration)
                                # pass  # TODO: remove "pass" when you've completed this condition.
                else:
                    self.log_info("object not detected not moving")
        ...
        if dist_to_object > 0:
            ...
            # if not self.virtual_mode:
                ...
                while self.drone.location.global_relative_frame.alt > alt_thresh:
                    ...
                    #       or you can do a drone_lib.move_local as well.
                    drone_lib.small_move_down(self.drone, 0.5, 1)
                    # pass  # TODO: remove "pass" when you've completed this condition.
                ...
                while self.drone.location.global_relative_frame.alt > 3.20:
                    ...
                    # TODO: you can perform "drone_lib.small_move_down here,
                    #       or you can do a drone_lib.move_local as well.
                    drone_lib.small_move_down(self.drone, 0.3, 1)
                    # pass  # TODO: remove "pass" when you've completed this condition.
            ...
            pex03_utils.release_grip(self.drone, 2)
    ...
    def conduct_mission(self):
        ...
        while self.drone.armed:
            ...
            self.log_info(f"last drone heading: {self.last_heading_pos}, longitude: {self.last_lon_pos}, latitude:{self.last_lat_pos}, alt: {self.last_alt_pos}")
            ...
            if not self.object_identified:
                ...
                center, confidence, corner, radius, frm_display, bbox \
                    = obj_track.check_for_initial_target(frame, frm_display,
                                                         show_img = self.virtual_mode, in_debug=self.virtual_mode)
                ...
                center, confidence, corner, radius, frm_display, bbox = obj_track.track_with_confirm(frame, img_write = frm_display, show_img = self.virtual_mode,debug_mode=self.virtual_mode)
                if not confidence:
                ...
                cv2.putText(frm_display,
                                ...
                                (300, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.75,
                                (0, 0, 255), 2)
